...
from structures import Message, checkVar, isValidName
import re
...
class Ui_messageDialog(object):
    ...
    def removeVar(self, index):
        ...
        components = self.varScrollContainer.findChildren(widgetTypes, )
        for w in components:
            w.setParent(None)
        self.vars -= 1
    ...
    def addMsg(self):
        ...
        contents = self.varScrollContainer.findChildren( (QtWidgets.QLineEdit,QtWidgets.QComboBox), QtCore.QRegularExpression("var.*"))
        contents_names = [c.objectName() for c in contents]

        varShift = 0
        if msgType == "MessageSpatial2D":
            varShift = 2
        elif msgType == "MessageSpatial3D":
            varShift = 3

        for i in range(1, self.vars+1-varShift):
        ...
        if "Spacial" in msgType:
            interactRad = self.radiusEdit.text()
            min = self.minEdit.text()
            max = self.maxEdit.text()
            if not checkVar(interactRad, "float"):
                self.errorMsg("Invalid parameter input")
                return
            
            minVals = min.split(",")
            for val in minVals:
                if not checkVar(val.strip(), "float"):
                    self.errorMsg("Invalid parameter input")
                    return
            if len(minVals) != msgType[-2]:
                self.errorMsg("Not enough valies were input (or in the wrong format)")
                return
        ...
        if "Bucket" in msgType:
            ...
            if (not checkVar(minBound, "int")) or (not checkVar(maxBound, "int")):
                self.errorMsg("Invalid parameter input")
                return
        ...
        if "Array" in msgType:
            ...
            print(len(dimensionVals))
            for val in dimensionVals:
                if not checkVar(val.text(), "UInt32"):
                    self.errorMsg("Invalid parameter input")
                    return
